...
export async function generateMemorySummary(entryContent: string, yearDifference: number) {
  try {
    const response = await openai.chat.completions.create({
      model: "gpt-4o",
      messages: [
...
export async function generateMonthlyInsights(entries: { content: string; mood?: string }[]) {
  try {
    // Extract text and moods from entries
    const entriesText = entries.map(e => `Content: ${e.content}\nMood: ${e.mood || "Unknown"}`).join("\n\n");
      ...
      messages: [
        {
          role: "system",
          content: 
            "You are an AI assistant specialized in analyzing patterns in journal entries. " +
            "Based on the provided journal entries from the past month, create: " +
        ...
        {
          role: "user",
          content: `Here are the journal entries from the past month:\n\n${entriesText}`
        }
      ],
      response_format: { type: "json_object" }
    });
    ...
    return {
      monthlyReflection: "Your journal entries from the past month show some interesting patterns.",
      suggestions: [
