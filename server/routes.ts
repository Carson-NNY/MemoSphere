...
// e.g. max 5 calls per user per minute
const openAiLimiter = rateLimit({
  ...
  keyGenerator: (req: Request) => {
    // if req.user.id is a number, convert it:
  ...
  app.post("/api/firebase-auth", async (req, res, next) => {
    try {
      ...
      if (!user) {
        ...
        const usernameBase =
          firebaseData.email?.split("@")[0] ||
          `user_${firebaseData.uid.substring(0, 8)}`;
        ...
        if (
          firebaseData.displayName !== undefined &&
          firebaseData.displayName !== user.displayName
        ) {
        ...
        if (
          firebaseData.email !== undefined &&
          firebaseData.email !== user.email
        ) {
        ...
        if (
          firebaseData.photoURL !== undefined &&
          firebaseData.photoURL !== user.photoURL
        ) {
        ...
        if (hasUpdates && "updateUser" in storage) {
          try {
            const updatedUser = await (storage as any).updateUser(
              user.id,
              updates
            );
            if (updatedUser) {
      ...
      if (!user) {
        return res
          .status(500)
          .json({ message: "Failed to create or retrieve user account" });
      }
      ...
      try {
        ...
        if (entryData.content) {
          ...
          if (
            !entryData.mood &&
            sentimentAnalysis &&
            typeof sentimentAnalysis === "object" &&
            "primaryEmotion" in sentimentAnalysis
          ) {
    ...
    async (req, res, next) => {
      try {
        ...
        if (recentEntries.length === 0) {
          return res.json({
            monthlyReflection:
              "You haven't created any journal entries in the past month.",
            suggestions: [
